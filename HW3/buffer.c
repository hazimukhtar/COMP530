...
void createBuffer(Buffer* buff)
{
	...
	sem_init(&buff->emptySem, 0, 20); // set the semaphore for the empty semaphore to 20
	sem_init(&buff->fullSem,0, 0); // set the full semaphore to the value of 0
...
void deposit(Buffer* buff, char c)
{
	...
	buff->in = (buff->in +1) % 20; // increase the in location by one and mod by 20 to make it circular
...
char remoove(Buffer* buff)
{
	...
	buff->out = (buff->out+1)%20; // this increases the value of the out and mod 20 to keep the count going around the buffer
